...
class ChangeNickname(commands.Cog):
    ...
    async def nickname(self, ctx, member: discord.Member, *, nickname):
        if ctx.guild.me.top_role < member.top_role:
            ...
            embed.add_field(name="• Nickname command", value=f"{member.mention}'s → Nickname has been **Changed!** ")
    ...
    async def nickname_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            embed.add_field(name="→ Invalid Argument!",
                            value="• Please put a valid option! Example: `l!nickname @user <nickname>`")
            await ctx.send(embed=embed)
